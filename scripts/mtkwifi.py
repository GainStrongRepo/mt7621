...
def patch_clear():
	...
	if os.path.exists(series):
		with open(series) as fd:
			for patch in reversed(fd.read().split()):
				if re.match(r"mt76\w{2,3}/.+patch", patch):
					patch_delete(patch)
	...
	for driver in drivers:
		if not re.match(r"mt76.{2,3}", driver): continue
...
def init_drivers_from_tarball(driver,pkgsrc):
	...
	cmd = """tar {tarcmd} {topdir}/dl/{pkgsrc} -C {destdir}""".format(
		...
		destdir=os.path.join(linuxdir,"drivers","net","wireless","mtk",driver))
	execute(cmd)
	patch_apply(driver)
...
def init_drivers():
	...
	for driver in drivers:
		...
		if not re.match(r"mt76.{2,3}", driver) or not os.path.exists(makefile):
			continue
		found = 0
