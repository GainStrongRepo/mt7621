...
ifeq ($(strip $(CONFIG_EXTERNAL_KERNEL_TREE)),"")
  ...
  define Kernel/Prepare/Default
	...
	git clone --depth=1 file://$(CONFIG_EXTERNAL_KERNEL_TREE) $(LINUX_DIR)
...
# copy CONFIG_KERNEL_* settings over to .config.target
	...
	if [ -f $(SCRIPT_DIR)/kconfig.py ]; then \
		$(SCRIPT_DIR)/kconfig.py -m -o 0 $(LINUX_DIR)/.config.target $(LINUX_DIR)/.config.override > $(LINUX_DIR)/.config; \
	else \
