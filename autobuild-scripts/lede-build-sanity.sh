...
copy_main_Config(){
	...
	echo cp -rfa autobuild-scripts/$1/.config ./.config
	cp -rfa autobuild-scripts/$1/.config ./.config
...
copy_additional_Config(){
	...
	if [ -f autobuild-scripts/$1/$4 ]; then
		
		echo cp -rfa autobuild-scripts/$1/$4 ./target/linux/$3/mt${2}/$4
		cp -rfa autobuild-scripts/$1/$4 ./target/linux/$3/mt${2}/$4
...
install_output_Image() {
	...
	bin_files=`find bin/$2 -name "*sysupgrade.bin"`
	file_count=0

	for file in $bin_files
	do
		tmp=${file%%-*}
		cp -rf $file ${INSTALL_DIR}/$1/${tmp##*/}-$1-${BUILD_TIME}.bin
		((file_count++))
        done
...
install_output_Config() {
	...
	echo cp -rfa autobuild-scripts/$1/.config ${INSTALL_DIR}/$1/openwrt.config
	cp -rfa autobuild-scripts/$1/.config ${INSTALL_DIR}/$1/openwrt.config
...
install_output_KernelDebugFile() {
	...
	KernelDebugFile=bin/$2/kernel-debug.tar.bz2
...
build() {
	...
	temp1=`grep "CONFIG_TARGET_ramips=y" autobuild-scripts/$1/.config`
	if [ "${temp1}" == "CONFIG_TARGET_ramips=y" ]; then
		arch_name="ramips"
		kernel_config_file="config-3.10"
	...
	# disable this since it's not standard
	# echo make kernel_olddefconfig
