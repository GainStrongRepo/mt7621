...
config6855Esw()
{
	...
	if [ "$CONFIG_P5_RGMII_TO_MT7530_MODE" == "y" ]; then
		echo "7620_P5 link witch 7530"
		switch reg w 2504 20ff0003
		switch reg w 2510 8100000

		switch_7530 reg w 2604 20ff0003
		switch_7530 reg w 2610 8100000
	fi
	...
	if [ "$1" = "LLLLW" ]; then
		...
		#VLAN member port
			if [ "$CONFIG_P5_RGMII_TO_MT7530_MODE" == "y" ]; then
				...
				switch_7530 reg w 2214 10001
				switch_7530 reg w 2314 10001
				switch_7530 reg w 2414 10002
				switch_7530 vlan set 0 1 11110011
				switch_7530 vlan set 0 2 00001011
			else
				switch vlan clear
		...
		#VLAN member port
			if [ "$CONFIG_P5_RGMII_TO_MT7530_MODE" == "y" ]; then
				...
				switch_7530 vlan set 0 1 01111111
				switch_7530 vlan set 1 2 10000011
			else
				switch vlan clear
