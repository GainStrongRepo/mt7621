...
vr_sta_entry_struct g_vr_sta_list[MAX_VR_STA_NUM];
UINT8 regrp_pending;
...
void vr_mlme_search_best_ap(p_sta_entry_struct p_entry)
{
...
void vr_mlme_handle_sta_message(UINT8 *buf, UINT32 buf_len)
{
 // Handle driver messages.
}

/*update the sta mlme state*/
void vr_mlme_get_sta_mlme_state(p_sta_entry_struct p_entry)
{


}

p_vr_sta_entry_struct vr_mlme_search_vr_sta_by_mac(UINT8 * mac_addr)
{
	p_vr_sta_entry_struct p_entry = g_vr_sta_list;
	int i;
...
#if 0
p_sta_entry_struct vr_mlme_search_sta_by_mac(UINT8 * mac_addr)
{
...
#if 0
p_vr_sta_entry_struct vr_mlme_add_vr_sta(p_msg_1_struct p_msg_1)
{
...
void  vr_mlme_del_vr_sta(p_vr_sta_entry_struct p_entry)
{
	memset(p_entry,0,sizeof(vr_sta_entry_struct));
}

#endif
p_repeater_list_struct vr_mlme_search_rept_by_ip(UINT32 ip_addr)
{
	int i=0;
...
void regrp_msg_2_timeout (void *eloop_data, void *user_ctx)
{
	...
	{
		vr_mlme_compose_send_msg_1(&g_own_rept_info, p_next_rept);
	}
	g_own_rept_info.need_retrigger_regrp =1;
	return;
}

UINT8 vr_mlme_compose_send_msg_regrp_initiate(p_repeater_list_struct p_rept)
{
	...
	p_rept->tcp_tx_sock = vr_tcp_connect(p_rept);
	...
	vr_tcp_disconnect(p_rept);
	return TRUE;
}

#if 0
vr_mlme_compose_send_msg_regrp_end(p_repeater_list_struct p_rept)
{
	msg_init_struct msg_init;
...
void vr_mlme_compose_send_msg_1(p_repeater_list_struct p_own_rept,p_repeater_list_struct p_rept)
{
	msg_1_struct msg_1;
...
void vr_mlme_compose_send_msg_2(p_repeater_list_struct p_own_rept,p_repeater_list_struct p_master_rept)
{
	UINT8 buffer[512] = {0};
	UINT16 buffer_len;
	msg_2_struct msg_2;
	...
	p_master_rept->tcp_tx_sock = vr_tcp_connect(p_master_rept);
	...
	vr_tcp_disconnect(p_master_rept);
...
void vr_mlme_compose_send_msg_3(p_repeater_list_struct p_rept, UINT8 status)
{
	msg_3_struct msg_3;
...
void regrp_scan_timeout(void *eloop_data, void *user_ctx)	//decide when to cancel
{
	...
	if(p_own_rept->is_master == 0)
	{
		...
		if(p_master_rept != NULL)
			vr_mlme_compose_send_msg_2(p_own_rept,p_master_rept);
	}
	else
	{
		//do nothing
...
vr_mlme_handle_msg_initiate(p_repeater_list_struct p_rept,p_msg_init_struct p_msg_1, UINT8 msg_len)
{
	printf("%s\n", __func__);
...
#if 0
vr_mlme_handle_msg_end(p_repeater_list_struct p_rept,p_msg_init_struct p_msg_1, UINT8 msg_len)
{
	p_repeater_list_struct p_own_rept = &g_own_rept_info;
	if(cli_interface_5g[0]!= '\0')
		set_regrp_mode(cli_interface_5g,NON_REGRP_MODE);
...
void vr_mlme_handle_msg_1(p_repeater_list_struct p_rept,p_msg_1_struct p_msg_1, UINT8 msg_len)
{
	p_repeater_list_struct p_own_rept = &g_own_rept_info;
	...
	vr_mlme_compose_send_msg_2(p_own_rept,p_rept);
...
//	UINT8 regrp_sent =0;
	...
	//	printf("isConnectedNonMAN:%d\n",is_connected_to_non_man(p_rept));
			...
			&& memcmp(p_rept->cli_target_mac_2g,ZERO_MAC_ADDR,MAC_ADDR_LEN) ==0)
		{
			...
			p_rept->regrp_state = REGRP_STATE_PEER_DONE;
			vr_mlme_compose_send_msg_3(p_rept,0);
		...
		if(p_rept->non_man_selected == 1 && 
			(MAC_ADDR_EQUAL(p_rept->cli_target_mac_5g, ZERO_MAC_ADDR) ||  memcmp(p_rept->cli_target_mac_5g,p_rept->cli_5g_bssid,MAC_ADDR_LEN) ==0
			&& (MAC_ADDR_EQUAL(p_rept->cli_target_mac_2g, ZERO_MAC_ADDR) || memcmp(p_rept->cli_target_mac_2g,p_rept->cli_2g_bssid,MAC_ADDR_LEN) ==0)))
		{
		...
		regrp_sent ++;
		vr_mlme_compose_send_msg_3(p_rept,regrp_sent);
...
void regrp_trigger_reconnection()
{
	...
	if(p_own_rept->non_ez_connection)
	{
		if((!MAC_ADDR_EQUAL(p_own_rept->cli_target_mac_5g, ZERO_MAC_ADDR)
			...
			||
			(!MAC_ADDR_EQUAL(p_own_rept->cli_target_mac_5g, ZERO_MAC_ADDR)
			&& !MAC_ADDR_EQUAL(p_own_rept->cli_target_mac_5g, p_own_rept->cli_5g_bssid)))
		{
	...
	if(!p_own_rept->non_ez_connection && regrp_sent>0 )
	{
		os_sleep(1,0);
		regrp_disable_apcli(p_own_rept);
	}
...
void vr_mlme_handle_msg_2(p_repeater_list_struct p_rept, p_msg_2_struct p_msg_2, UINT8 msg_len)
{
...
void vr_mlme_handle_msg_3(p_repeater_list_struct p_rept,p_msg_3_struct p_msg_3, UINT8 msg_len)
{
	p_repeater_list_struct p_own_rept = &g_own_rept_info;
...
int regrp_get_cand_list(UCHAR *ifname, UCHAR *cand_list, UCHAR *cand_count)
{
	...
	{
		printf("Couldnt get candidate list 2\n");
...
void event_scan_comp(struct _regrp_event *regrp_event_data)	//decide when to cancel
{
		...
		&& p_own_rept->scan_done_5g == 1)
	{
		...
		else
		{
			...
			if(p_rept)
			{
				vr_mlme_compose_send_msg_1(p_own_rept,p_rept);
...
UINT8 regrp_initiate()
{
	...
	{
		{
			if (vr_mlme_compose_send_msg_regrp_initiate(p_rept) == FALSE)
				return FALSE;
		}
		p_rept = regrp_get_next_rept(p_rept);
	}
...
//	regrp_clean_all_states();
	...
	vr_mlme_compose_send_msg_1(&g_own_rept_info,p_rept_entry);
