...
void vr_get_own_ap_info(p_ap_info_struct p_ap_info, char *intf_name)
{
	int sd;
...
#if 0
void vr_handle_driver_event(p_vr_event_t vr_event)
{
	switch(vr_event->event_id)
	{
		case OID_VR_EVENT_TRIGGER_HANDOFF_MR:
		{
			printf("OID_VR_EVENT_TRIGGER_HANDOFF_MR\n");
			//vr_handle_trigger_handoff_event(vr_event);
			break;
		}
#if 0
		case OID_VR_SEND_SNIFF_RESULT:
		{
			p_peer_sniff_result_t sniff_result = vr_event->event_body;
			...
			if (rept_entry)
			{
				...
				vr_tcp_send(rept_entry->tcp_tx_sock, buffer, sizeof(peer_sniff_result_t));
			}
			break;
		}
#endif
...
#endif
#if 0
void vr_dr_reset_handoff_trigger(p_sta_entry_struct sta_entry, UINT8 reason_code)
{
	int sd;
	char command_buf[256] = {0};
	p_vr_command_t vr_command = (p_vr_command_t)command_buf;
	vr_command->command_id = OID_VR_QUERY_CMD_RESET_HANDOFF_TRIGGER;
	memcpy(vr_command->command_body, sta_entry->mac_addr, MAC_ADDR_LEN);
	vr_command->command_body[MAC_ADDR_LEN] = reason_code;
	sd = socket(AF_INET, SOCK_DGRAM, 0);
...
void vr_dr_get_current_rssi(p_sta_entry_struct sta_entry)
{
	int sd;
	char command_buf[256] = {0};
	p_vr_command_t vr_command = (p_vr_command_t)command_buf;
	vr_command->command_id = OID_VR_QUERY_STA_CURRENT_RSSI;
	memcpy(vr_command->command_body, sta_entry->mac_addr, MAC_ADDR_LEN);
	sd = socket(AF_INET, SOCK_DGRAM, 0);
	if (sta_entry->band == Band_2G)
	{
...
void vr_dr_send_sniff_request(p_vr_sniff_req_t sniff_request)
{
	int sd;
	char command_buf[256] = {0};
	p_vr_command_t vr_command = (p_vr_command_t)command_buf;
	vr_command->command_id = OID_VR_CONFIG_SNIFF_MODE;
	memcpy(vr_command->command_body, sniff_request, sizeof(vr_sniff_request_t));
	sd = socket(AF_INET, SOCK_DGRAM, 0);
...
#if 0
int vr_drv_update_sta(p_vr_sta_entry_struct p_entry)
{
		//Maybe no need. can be done after VR3
}


void vr_drv_update_activate_vr_sta(p_vr_sta_entry_struct p_entry)
{
	...
	UINT16 buf_len=0;
	p_vr_command_t vr_command = (p_vr_command_t)command_buf;
	p_vr_sta_update_struct p_vr_update = (p_vr_sta_update_struct)vr_command->command_body;
	
	if(if_indextoname(p_entry->vr_rept_inf_idx, intf_name) == NULL)
	{
		perror("if_indextoname");
		return;
	}
	
	vr_command->command_id = OID_VR_UPDATE_VR_STA;
	memcpy(p_vr_update->mac_addr,p_entry->mac_addr, MAC_ADDR_LEN);
	p_vr_update->mlme_state_len = p_entry->mlme_state_len;
	memcpy(p_vr_update->sta_mlme_state, p_entry->sta_mlme_state, p_entry->mlme_state_len);

	buf_len = sizeof (vr_command)+ sizeof(vr_sta_update_struct) + p_entry->mlme_state_len;
...
void vr_drv_disable_sta(p_sta_entry_struct p_entry)
{
	int sd;
	UINT8 intf_name[IFNAMSIZ];
	...
	vr_command->command_id = OID_VR_DISABLE_STA;
	memcpy(vr_command->command_body,p_entry->mac_addr, MAC_ADDR_LEN);
...
void vr_drv_get_pn(p_sta_entry_struct p_entry)
{
	int sd;
	UINT8 intf_name[IFNAMSIZ];
	char command_buf[10] = {0};
	...
	vr_command->command_id = OID_VR_GET_STA_PN;
	memcpy(vr_command->command_body,p_entry->mac_addr, MAC_ADDR_LEN);
...
void vr_drv_get_sta_mlme_state(UINT8* mac_addr, UINT8 ifindex, UINT8 * mlme_state, UINT16 *mlme_state_len)
{
	int sd;
	UINT8 intf_name[IFNAMSIZ];
	...
	vr_command->command_id = OID_VR_GET_STA_MLME_STATE;
	memcpy(vr_command->command_body,mac_addr, MAC_ADDR_LEN);
