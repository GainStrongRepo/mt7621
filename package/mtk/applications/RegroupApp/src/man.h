...
#define DEFAULT_MAX_RSSI_THRESHOLD (-85)
#define CUSTOM_RSSI_TH (-60)
...
typedef struct vr_trigger_handoff_s
{
	UCHAR rssi;
	UCHAR band;
	UINT32 ip_addr;
	UCHAR mac_addr[MAC_ADDR_LEN];
} vr_trigger_handoff_t, *p_vr_trigger_handoff_t;

typedef struct vr_command_s
{
	UINT8 command_id;
	UINT8 command_body[0];
} vr_command_t, *p_vr_command_t;

typedef struct vr_event_s
{
	UINT8 event_id;
	UINT8 event_body[0];
} vr_event_t, *p_vr_event_t;

typedef struct vr_config_interface_s
{
	UINT8 main_idx;
	UINT8 vr_idx;
	UINT8 vr_init;
	UINT8 vr_mac[MAC_ADDR_LEN];
} vr_config_interface_t, *p_vr_config_interface_t;

typedef struct vr_sniff_request_struct
{
	UINT8 br_mac[MAC_ADDR_LEN];
	UINT8 client_mac[MAC_ADDR_LEN];
	UINT8 band;
}vr_sniff_request_t, *p_vr_sniff_req_t;
...
typedef struct sniff_result_s
{
	...
	p_repeater_list_struct vr_entry;
...
typedef struct vr_sta_entry
{
	UINT8 valid;
	UINT8 state;
	UINT8 vr_rept_inf_idx;
	UINT8 PN[PN_LEN];
	UINT32 ip_addr;
	UINT8 mac_addr[MAC_ADDR_LEN];
	UINT8 dialog_token;
...
} vr_sta_entry_struct, *p_vr_sta_entry_struct;

typedef struct vr_sta_update
{
	UINT8 mac_addr[MAC_ADDR_LEN];
	UINT16 mlme_state_len;
	UINT8 sta_mlme_state[0]; //driver specific
}vr_sta_update_struct, *p_vr_sta_update_struct;
