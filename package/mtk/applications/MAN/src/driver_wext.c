...
extern char apclii0_peer_mac[];

void man_driver_event_handle(char *buf);
void man_conf_event_handle(char *buf);
int update_group_id(char *data);

char is_dual_chip_dbdc;
extern char is_multipro;
extern unsigned char is_triband;
extern unsigned char is_man_nonman;
extern unsigned char device_connected_0;
extern unsigned char device_connected_1;
extern device_info_from_user_t input_device_info[MAX_BAND];

#ifdef SINGLE_BACKHAUL_LINK
void check_cli_status();
extern unsigned char roam_triggered_on_2_g;
extern unsigned char roam_triggered_on_5_g;
#endif
    ...
    while (pos + IW_EV_LCP_LEN <= end) {
        ...
        case IWEVCUSTOM:
			...
			case OID_WH_EZ_MAN_DEAMON_EVENT:
				...
				break;
			case OID_WH_EZ_MAN_TRIBAND_EZ_DEVINFO_EVENT:
				//hex_dump("Triband Event",buf,iwe->u.data.length);
				man_triband_ez_driver_event_handle(buf);
				break;
			case OID_WH_EZ_MAN_TRIBAND_NONEZ_DEVINFO_EVENT:
				//hex_dump("Triband Event",buf,iwe->u.data.length);
				man_triband_nonez_driver_event_handle(buf);
				break;
			case OID_WH_EZ_MAN_PLUS_NONMAN_EZ_DEVINFO_EVENT:
				//hex_dump("Man plus NON MAN Event",buf,iwe->u.data.length);
				man_plus_nonman_ez_driver_event_handle(buf);
				break;
			case OID_WH_EZ_MAN_PLUS_NONMAN_NONEZ_DEVINFO_EVENT:
				//hex_dump("MAN plus NON MAN Event",buf,iwe->u.data.length);
				man_plus_nonman_nonez_driver_event_handle(buf);
				break;							
			case OID_WH_EZ_UPDATE_STA_INFO:
				man_update_station_event(buf, iwe->u.data.length / 6);
				break;
			case WHC_DRVEVNT_STA_JOIN:
				hex_dump("MAN WHC_DRVEVNT_STA_JOIN Event",buf,8);
				man_add_station_event(buf);
				break;
			case WHC_DRVEVNT_STA_LEAVE:
				hex_dump("MAN WHC_DRVEVNT_STA_LEAVE Event",buf,8);
				man_delete_station_event(buf);
				break;
			case WHC_DRVEVNT_EXT_UPLINK_STAT:
				hex_dump("MAN WHC_DRVEVNT_EXT_UPLINK_STAT Event",buf,8);
				if(is_triband || is_man_nonman) {
					get_connected_ap_mac(apclii0_peer_mac, input_device_info[get_ez_idx()].cli_interface);
				} 
				else {
#ifdef SINGLE_BACKHAUL_LINK	
				...
				printf("2.4G connected?%c, 5G connected?%c\n", device_connected_0, device_connected_1);

				if((device_connected_1 == 1))
				{
					...
					if ((roam_triggered_on_2_g == 0) && (roam_triggered_on_5_g == 0)){ 
						...
						printf("CLI disable due to roam action, do not start again\n");
					}
				} else if ((device_connected_1 == 0) && (device_connected_0 == 1)){
					printf("2.4G Connect, 5G not connected, check once again in 40 secs\n");
					memset(command,0,200);
					sprintf(command,"iwpriv %s set ApCliEnable=1",input_device_info[1].cli_interface);
					system(command);
					eloop_register_timeout(40, 0, check_cli_status, NULL, NULL); 	
				}
				if (roam_triggered_on_5_g && device_connected_1)
				{
					printf("5G successfully roamed\n");
					roam_triggered_on_5_g = 0;
				} 
				if (roam_triggered_on_2_g && device_connected_0)
				{
					printf("2.4G successfully roamed\n");
	            ...
	            device_connected_0 = is_get_connected(input_device_info[0].cli_interface);
                device_connected_1 = is_get_connected(input_device_info[1].cli_interface);
				DBGPRINT(DEBUG_ERROR, "%s 2.4G connected? %d, 5G connected? %d\n",
							   __FUNCTION__, device_connected_0, device_connected_1);

#endif
			}
				break;
			case OID_WH_EZ_MAN_TRIBAND_SCAN_COMPLETE_EVENT:
				hex_dump("Triband Event",buf,6);
				man_triband_scan_complete_event(buf);
				break;
			case OID_WH_EZ_MAN_CONF_EVENT:
				man_conf_event_handle(buf);
				break;	
#ifdef REGROUP_SUPPORT
			case OID_WH_EZ_REGROUP_EVENT:
				...
				break;
#endif
			case OID_WH_EZ_GROUP_ID_UPDATE:
				update_group_id(buf);
				break;
#ifdef REGROUP_SUPPORT
			case OID_WH_EZ_REGROUP_DBG_LEVEL_EVENT:
				Set_regroup_dbglevel_event_handle(buf);
				break;
			case OID_WH_EZ_REGROUP_THRESHOLD_EVENT:
				Set_regroup_threshold_event_handle(buf);
				break;
			case OID_WH_EZ_REGROUP_SHOW_CANDIDATE_RSSI_EVENT:
				Set_regroup_show_candidate_rssi_event_handle(buf);
				break;
#endif
			default:
