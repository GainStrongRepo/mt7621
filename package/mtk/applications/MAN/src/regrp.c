...
signed int tcp_listeners[MAX_VIRTUAL_REPT_NUM];
UINT32 g_virtual_repeater_count;
struct os_time last_entry_added_time;
UINT8 trigger_regrp =1;
int regrp_udp_5005_sock = 0;
int regrp_tcp_5005_sock = 0;

/*************************************************************************************************/

/*Extern*/
extern unsigned char ZERO_MAC_ADDR[MAC_ADDR_LEN];
char interface_5g[16];
char interface_2g[16];
extern UINT8 regrp_pending;
char cli_interface_5g[16];
char cli_interface_2g[16];
...
void regrp_set_own_rept_info()
{
	memzero(&g_own_rept_info, sizeof (g_own_rept_info));
	if(interface_5g[0] != '\0')
	{
		regrp_get_own_ap_info(&g_own_rept_info.ap_info_5g, interface_5g);
		ez_hex_dump("5G Info",&g_own_rept_info.ap_info_5g,sizeof(ap_info_struct));
		
	}
	if(interface_2g[0] != '\0')
	{
		regrp_get_own_ap_info(&g_own_rept_info.ap_info_2g, interface_2g);
...
int regrp_tcp_disconnect(p_repeater_list_struct p_rept_entry)
{
	printf("%s: %x\n",__func__,p_rept_entry->ip_addr);
	if(p_rept_entry->tcp_tx_sock >=0)
	{
		close(p_rept_entry->tcp_tx_sock);
		p_rept_entry->tcp_tx_sock = -1;
		p_rept_entry->tcp_link_done = 0;
	}
}

int regrp_tcp_connect(p_repeater_list_struct p_rept_entry)
{
	int sockfd, n;
...
int regrp_tcp_send(int sockfd, UINT8 *buffer)
{
}
#if 0
void regrp_send_command_repeater_config(p_repeater_list_struct p_rept_entry, UINT8 band)
{
	UINT8 buffer[256] = {0};
	p_regrp_command_t regrp_command = (p_regrp_command_t)buffer;
	p_regrp_config_interface_t regrp_config = (p_regrp_config_interface_t)regrp_command->command_body;
	int sd;

	sd = socket(AF_INET, SOCK_DGRAM, 0);
	
	regrp_command->command_id =  OID_VR_CONFIG_VIRTUAL_INTERFACE;
	if (band == Band_2G)
	{
		regrp_config->main_idx = g_own_rept_info.ap_info_2g.wdev_id;
		regrp_config->regrp_idx = p_rept_entry->ap_info_2g.wdev_id;
		regrp_config->regrp_init = 1;
		memcpy(regrp_config->regrp_mac, p_rept_entry->ap_info_2g.mac_addr, MAC_ADDR_LEN);		
		ap_oid_query_info(OID_VIRTUAL_ROAM_COMMAND, sd, interface_2g, buffer, sizeof(*regrp_config) + 1);
		...
		regrp_config->main_idx = g_own_rept_info.ap_info_5g.wdev_id;
		regrp_config->regrp_idx = p_rept_entry->ap_info_5g.wdev_id;
		regrp_config->regrp_init = 1;
		memcpy(regrp_config->regrp_mac, p_rept_entry->ap_info_5g.mac_addr, MAC_ADDR_LEN);
		if (ap_oid_query_info(OID_VIRTUAL_ROAM_COMMAND, sd, interface_5g, buffer, sizeof(*regrp_config) + 1))
		{
...
#endif
void regrp_set_peer_device_shared_info (p_device_shared_info_struct p_new_rept, p_repeater_list_struct rept_entry)
{
...
void regrp_add_rept_entry (p_device_shared_info_struct new_rept_entry)
{
...
void regrp_delete_rept_entry(p_repeater_list_struct p_rept_entry)
{
...
void regrp_close_all_tcp_listners()
{
	int i = 0;
	for (i = 0; i < MAX_VIRTUAL_REPT_NUM; i++)
	{
...
void regrp_close_all_tcp_tx_sockets()
{
	int i = 0;
...
void regrp_get_own_device_shared_info (p_device_shared_info_struct p_device_info)
{
	UINT32 local_ip;
	...
	if (local_ip != g_own_rept_info.ip_addr)
	{
		...
		regrp_close_all_tcp_tx_sockets();
		os_get_time(&last_entry_added_time);
	}
...
static void regrp_server(int sock, void *eloop_ctx, void *sock_ctx)
{
...

	{
		...
		if (!memcmp(&buf[0], REPT_INFO_TAG, REPT_INFO_TAG_LEN))
		{
			...
			//	hex_dump("UDP_MSG", &buf[REPT_INFO_TAG_LEN],6);
				...
				if (virtual_rept_info == NULL)
				{
					...
					regrp_add_rept_entry(rept_info);
				} else {
					...
					if(new_rept_info)
					{
						...
						if (new_rept_info & IP_CHANGED) {
							...
							//regrp_tcp_disconnect(virtual_rept_info);
						}
					...
					if (new_rept_info 
						...
						&& virtual_rept_info->tcp_tx_sock)
						{
							...
							if (new_rept_info & IP_CHANGED) {
								...
								virtual_rept_info->ip_addr = rept_info->ip_addr;
								regrp_tcp_disconnect(virtual_rept_info);
								//regrp_tcp_connect(virtual_rept_info);
		...
		{
			UINT8 msg_len = message - REPT_INFO_TAG_LEN;
			p_regrp_sniff_req_t sniff_request = (p_regrp_sniff_req_t)(&buf[SNIFF_REQ_TAG_LEN]);
			p_repeater_list_struct virtual_rept_info = NULL;
			if (!MAC_ADDR_EQUAL(sniff_request->br_mac, g_own_rept_info.br_mac))
			{
				...
				{
					regrp_dr_send_sniff_request(sniff_request);
				} else {
...
void regrp_rept_update_tcp_sock(int sock, UINT8 add)

{
	...
	if((p_rept_list = regrp_mlme_search_rept_by_ip(address.sin_addr.s_addr)) != NULL)
	{
		if (add)
...
static void regrp_handle_rx_tcp_pkt(p_repeater_list_struct p_rept,UINT8 *buffer, UINT32 valread)
{
	switch (buffer[MAC_ADDR_LEN])
	{
		case MSG_REGRP_INIT:
			regrp_mlme_handle_msg_initiate(p_rept,buffer,valread);
			break;
		case MSG_1:  // start_regroup and scan procedure
			regrp_mlme_handle_msg_1(p_rept,buffer,valread);
			break;
		case MSG_2: // handle scan results of all repeaters
			regrp_mlme_handle_msg_2(p_rept,buffer,valread);
			break;
		case MSG_3: // handle the connect command. 
			regrp_mlme_handle_msg_3(p_rept,buffer,valread);
			break;
		case MSG_REGRP_END: // handle the connect command. 
			regrp_mlme_handle_msg_4(p_rept,buffer,valread);
			break;
		default:
...
static void regrp_tcp_cli_server(int sock, void *eloop_ctx, void *sock_ctx)
{
	struct sockaddr_in address;
	UINT32 addrlen;
	ssize_t  valread;
	...
	hex_dump("PeerName", (UCHAR *)&address, sizeof(address));
	...
	//   printf("Host disconnected , ip %s , port %d \n" , ip, ntohs(address.sin_port));
	   //regrp_rept_update_tcp_sock(sock, FALSE);
	...
	{
		regrp_handle_rx_tcp_pkt(p_rept,buffer, valread);
...
static void regrp_tcp_main_server(int sock, void *eloop_ctx, void *sock_ctx)
{
...
//	   exit(EXIT_FAILURE);
	...
	//regrp_rept_update_tcp_sock(tcp_cli_srv_sock, TRUE);
	for (i = 0; i < MAX_VIRTUAL_REPT_NUM; i++)
	{
		if (tcp_listeners[i] < 0)
		{
	...
	eloop_register_read_sock(tcp_cli_srv_sock,regrp_tcp_cli_server,NULL,NULL);
}

/*Register the TCP and UDP server to listen for messages from other repeaters.*/
void regrp_register_server()
{
	...
	regrp_udp_5005_sock = server_socket=socket(AF_INET, SOCK_DGRAM, 0);

	printf("%s: create regrp_5005_socket %d\n", __FUNCTION__, server_socket);
	...
	if( setsockopt(server_socket, SOL_SOCKET, SO_REUSEADDR, (char *)&opt, sizeof(opt)) < 0 )
   	{
   		perror("setsockopt");
		printf("%s: setsockopt regrp_5005_socket SO_REUSEADDR error\n", __FUNCTION__);
		close(server_socket);
		exit(EXIT_FAILURE);
	}
	...
	if( setsockopt(server_socket, SOL_SOCKET, SO_REUSEPORT, (char *)&opt, sizeof(opt)) < 0 )
   	{
   		perror("setsockopt");
		printf("%s: setsockopt regrp_5005_socket SO_REUSEPORT error\n", __FUNCTION__);
		close(server_socket);
		exit(EXIT_FAILURE);
	}
	...
	if(checkCall == -1) {
		...
		printf("%s: bind regrp_5005_socket error %d\n", __FUNCTION__,errno);
		close(server_socket);
		exit(EXIT_FAILURE);
	}

	eloop_register_read_sock(server_socket,regrp_server,NULL,NULL);

	/*create socket to listen to unicast frames*/
	
	regrp_tcp_5005_sock = server_tcp_socket=socket(AF_INET, SOCK_STREAM, 0);
    ...
    {
		perror("setsockopt");
		printf("%s: setsockopt server_tcp_socket SO_REUSEADDR  error\n", __FUNCTION__);
		close(server_socket);
                close(server_tcp_socket);
		exit(EXIT_FAILURE);
    }

    if( setsockopt(server_tcp_socket, SOL_SOCKET, SO_REUSEPORT, (char *)&opt, sizeof(opt)) < 0 )
    {
		perror("setsockopt");
		printf("%s: setsockopt server_tcp_socket SO_REUSEPORT error\n", __FUNCTION__);
		close(server_socket);
                close(server_tcp_socket);
		exit(EXIT_FAILURE);
    }
	...
	if(checkCall == -1) {
		perror("Error: bind call failed");
		printf("%s: bind server_tcp_socket  error\n", __FUNCTION__);
		close(server_socket);
		close(server_tcp_socket);
		exit(EXIT_FAILURE);
	}
	...
	{
		perror("listen");
		printf("%s: listen server_tcp_socket  error\n", __FUNCTION__);
		close(server_socket);
		close(server_tcp_socket);
		exit(EXIT_FAILURE);
	}
	...
	eloop_register_read_sock(server_tcp_socket,regrp_tcp_main_server,NULL,NULL);
...
UINT8 is_mac_highest(p_repeater_list_struct p_own_rept)
{
	...
	hex_dump("[DAEMON]OwnMAC:", p_own_rept->br_mac,6);
	for (i = 0; i < MAX_VIRTUAL_REPT_NUM; i++)
	{
...
UINT8 is_regrp_master_candidate(p_repeater_list_struct p_own_rept)
{
	...
	if(is_root_node(&p_own_rept->node_number))
	{
		//printf("[DAEMON]Is root Node\n");
	...
	{
		//printf("[DAEMON]Connected to Non MAN : IP : %x: vrtRcnt: %d\n",g_own_rept_info.ip_addr,g_virtual_repeater_count);
...
UINT8 should_trigger_regrp_first_time(p_repeater_list_struct p_own_rept)
{
	...
	os_get_time(&now);
		...
		if(((g_virtual_repeater_count > 1)
			...
			(now.sec > last_entry_added_time.sec
				&& now.sec - last_entry_added_time.sec > 60)))
#else
		if((g_virtual_repeater_count > 1 && 
			(now.sec > last_entry_added_time.sec
...
UINT8 should_trigger_regrp(p_repeater_list_struct p_own_rept)
{
	...
	if (trigger_regrp == 0)
	{
		//DBGPRINT(DEBUG_OFF, "%s <=== trig regrp 0 \n",__FUNCTION__);
		return;
	}
		//DBGPRINT(DEBUG_OFF, "%s %d ===> \n",__FUNCTION__,__LINE__);
...
//	DBGPRINT(DEBUG_OFF, "%s %d ===> \n",__FUNCTION__,__LINE__);
	...
	{
		trigger_regrp = 0;
		printf("[DAEMON]Trigger Regroup first time %d\n",g_virtual_repeater_count);
		trigger_regroup();
		return;
	}
...
void regrp_maintainance()
{
	...
	for (i = 0; i < MAX_VIRTUAL_REPT_NUM; i++)
	{
		...
		)
		{
			...
			regrp_delete_rept_entry(virtual_rept);
			os_get_time(&last_entry_added_time);
		}
		if(virtual_rept->valid 
					&& virtual_rept->regrp_init_done 
		...
		if (virtual_rept->valid 
			...
			//	virtual_rept->tcp_tx_sock = regrp_tcp_connect(virtual_rept);	
...
void regrp_udp_client()
{
	...
	//printf("udp send\n");
	{
		...
		*/ 
		...
		//memcpy(&config[REPT_INFO_TAG_LEN], regrp_get_own_device_shared_info(), sizeof(repeater_list_struct));
		regrp_get_own_device_shared_info((p_device_shared_info_struct)&config[REPT_INFO_TAG_LEN]);
...
void regrp_periodic_exec()
{
	//printf("[DAEMON===>]regrp_periodic_exec\n");
	regrp_udp_client();
	regrp_maintainance();
	
	should_trigger_regrp(&g_own_rept_info);
//	DBGPRINT(DEBUG_OFF, "%s ===>\n",__FUNCTION__);
	eloop_register_timeout(1, 0, regrp_periodic_exec, NULL, NULL);
	//printf("[DAEMON <===]regrp_periodic_exec\n");
}
void regrp_terminate(int sig, void *signal_ctx)
{
...
void regrp_send_sniff_req(p_regrp_sniff_req_t sniff_request)
{
	int sock1, sinlen, config_len;
	char broadcast_mac[MAC_ADDR_LEN] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
	char config[256] = {"\0"};
	...
	
	{
		...
		*/ 
		...
		memcpy(&config[SNIFF_REQ_TAG_LEN], sniff_request, sizeof(regrp_sniff_request_t));
		//regrp_get_own_device_shared_info(&config[REPT_INFO_TAG_LEN]);
...
void regrp_handle_rssi_too_low(char * buf)
{
	
}

void regrp_handle_rssi_better(char * buf)
{
	
}

void regrp_update_station_event(char * buf, int num_sta)
{
	int i = 0;
	int j = 0;
...
void regrp_delete_station_event(char * buf)
{
	int i = 0;
	unsigned char ZERO_MAC_ADDR[MAC_ADDR_LEN] = {0};
	for (i = 0; i < MAX_STA_SUPPORT; i++)
...
int regrp_find_best_ap_for_sta(p_sta_entry_struct sta_entry)
{
	int i =0;
...
void regrp_sniff_req_timeout(void *eloop_data, void *user_data)
{
	int target_repeater_idx = 0;
	p_sta_entry_struct sta_entry = user_data;
	printf("regrp_sniff_req_timeout\n");
	if (sta_entry->state == VR_STA_STATE_SEARCH)
	{
		if (sta_entry->sniff_cb.sniff_results_count == g_virtual_repeater_count)
		{
			...
			regrp_dr_get_current_rssi(sta_entry);
			target_repeater_idx  = regrp_find_best_ap_for_sta(sta_entry);
			if (target_repeater_idx == -1)
			{
				...
				regrp_dr_reset_handoff_trigger(sta_entry, NO_BETTER_REPEATER_FOUND);
			} else {
			...
			sta_entry->state = VR_STA_STATE_OWN;
			regrp_dr_reset_handoff_trigger(sta_entry, RESULTS_INCOMPLETE);
		}
	} else {
...
void regrp_force_ping_timeout(void *eloop_data, void *user_data)
{
	p_sta_entry_struct sta_entry = user_data;
	UINT8 * ip = &sta_entry->ip_addr;
	UINT8 ping[250] = {'\0'};
	//printf("regrp_force_ping_timeout\n");
	sprintf(ping, "ping -c1 %d.%d.%d.%d -w 2 > t", ip[0], ip[3], ip[2], ip[3]);
	system(ping);
	if (sta_entry->state == VR_STA_STATE_SEARCH)
		eloop_register_timeout(0,200,regrp_force_ping_timeout,NULL,sta_entry);
	
}
void regrp_handle_trigger_handoff_event(p_regrp_event_t regrp_event)
{
	regrp_sniff_request_t sniff_req;
	int sta_index;
	p_regrp_trigger_handoff_t trigger_handoff= regrp_event->event_body;
	if (regrp_mlme_search_sta_by_mac(trigger_handoff->mac_addr) == NULL)
	{
		printf("Sta Entry not available\n");
		sta_index = regrp_get_station_entry();
		if (sta_index != -1)
		{
	...
	regrp_send_sniff_req(&sniff_req);
	eloop_register_timeout(0,200,regrp_force_ping_timeout,NULL,&g_station_list[sta_index]);
	eloop_register_timeout(5,0,regrp_sniff_req_timeout,NULL,&g_station_list[sta_index]);
}

void regrp_send_msg_to_rept(p_repeater_list_struct p_rept, UINT8 *buf, UINT8 buf_len)
{
