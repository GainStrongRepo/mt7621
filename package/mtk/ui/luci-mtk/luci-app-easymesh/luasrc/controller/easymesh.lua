...
function index()
    ...
    entry({"admin", "network", "test"}, call("test"))
    ...
    if map_help then
        ...
        entry({"admin", "network", "multi_ap"}, template("admin_network/mtk_wifi_multi_ap"), _("EasyMesh"), 5);
        entry({"admin", "network", "multi_ap", "map_cfg"}, call("map_cfg")).leaf = true;
        entry({"admin", "network", "multi_ap", "get_device_role"}, call("get_device_role")).leaf = true;
        entry({"admin", "network", "multi_ap", "trigger_mandate_steering_on_agent"}, call("trigger_mandate_steering_on_agent")).leaf = true;
        entry({"admin", "network", "multi_ap", "trigger_back_haul_steering_on_agent"}, call("trigger_back_haul_steering_on_agent")).leaf = true;
        entry({"admin", "network", "multi_ap", "trigger_wps_fh_agent"}, call("trigger_wps_fh_agent")).leaf = true;
        entry({"admin", "network", "multi_ap", "display_runtime_topology"}, template("admin_network/mtk_wifi_map_runtime_topology")).leaf = true;
        entry({"admin", "network", "multi_ap", "get_runtime_topology"}, call("get_runtime_topology")).leaf = true;
        entry({"admin", "network", "multi_ap", "trigger_multi_ap_on_boarding"}, call("trigger_multi_ap_on_boarding")).leaf = true;
        entry({"admin", "network", "multi_ap", "display_client_capabilities"}, template("admin_network/mtk_wifi_map_client_capabilities")).leaf = true;
        entry({"admin", "network", "multi_ap", "get_client_capabilities"}, call("get_client_capabilities")).leaf = true;
        entry({"admin", "network", "multi_ap", "display_ap_capabilities"}, template("admin_network/mtk_wifi_map_ap_capabilities")).leaf = true;
        entry({"admin", "network", "multi_ap", "trigger_uplink_ap_selection"}, call("trigger_uplink_ap_selection")).leaf = true;
        entry({"admin", "network", "multi_ap", "get_bh_connection_status"}, call("get_bh_connection_status")).leaf = true;
        entry({"admin", "network", "multi_ap", "get_sta_steering_progress"}, call("get_sta_steering_progress")).leaf = true;
        entry({"admin", "network", "multi_ap", "get_al_mac"}, call("get_al_mac")).leaf = true;
        entry({"admin", "network", "multi_ap", "apply_ap_steer_rssi_th"}, call("apply_ap_steer_rssi_th")).leaf = true;
        entry({"admin", "network", "multi_ap", "apply_channel_utilization_th"}, call("apply_channel_utilization_th")).leaf = true;
        entry({"admin", "network", "multi_ap", "get_sta_bh_interface"}, call("get_sta_bh_interface")).leaf = true;
        entry({"admin", "network", "multi_ap", "get_ap_bh_inf_list"}, call("get_ap_bh_inf_list")).leaf = true;
        entry({"admin", "network", "multi_ap", "get_ap_fh_inf_list"}, call("get_ap_fh_inf_list")).leaf = true;
        entry({"admin", "network", "multi_ap", "display_fh_status_bss"}, template("admin_network/mtk_wifi_map_bssinfo")).leaf = true;
        entry({"admin", "network", "multi_ap", "display_bh_link_metrics_ctrler"}, template("admin_network/mtk_wifi_map_bh_link_metrics")).leaf = true;
        entry({"admin", "network", "multi_ap", "easymesh_bss_config_renew"}, template("admin_network/mtk_wifi_map_bss_cfg_renew")).leaf = true;
        entry({"admin", "network", "multi_ap", "easymesh_bss_cfg"}, call("easymesh_bss_cfg")).leaf = true;
        entry({"admin", "network", "multi_ap", "validate_add_easymesh_bss_req"}, call("validate_add_easymesh_bss_req")).leaf = true;
        entry({"admin", "network", "multi_ap", "remove_easymesh_bss_cfg_req"}, call("remove_easymesh_bss_cfg_req")).leaf = true;
        entry({"admin", "network", "multi_ap", "apply_easymesh_bss_cfg"}, call("apply_easymesh_bss_cfg")).leaf = true;
  
        entry({"admin", "network", "multi_ap", "apply_reboot"}, template("admin_network/mtk_wifi_apply_reboot")).leaf = true;
        entry({"admin", "network", "multi_ap", "loading"}, template("admin_network/mtk_wifi_loading")).leaf = true;
   	entry({"admin", "network", "multi_ap", "get_apply_status"}, call("get_apply_status")).leaf = true;
        entry({"admin", "network", "multi_ap", "reload"}, call("reload_wifi")).leaf = true;
        entry({"admin", "network", "multi_ap", "apply_wifi_bh_priority"}, call("apply_wifi_bh_priority")).leaf = true;
...
function reload_wifi(devname)
    mtkwifi.__run_in_child_env(__mtkwifi_reload, devname)
    local url_to_visit_after_reload = luci.dispatcher.build_url("admin", "network", "multi_ap")
    luci.http.redirect(luci.dispatcher.build_url("admin", "network", "multi_ap", "loading",url_to_visit_after_reload))
end
...
function reset_to_default_easymesh()
    ...
    luci.http.redirect(luci.dispatcher.build_url("admin", "network", "multi_ap", "loading",mtkwifi.get_referer_url()))
end
...
function map_cfg()
    ...
    for k,v in pairs(http.formvalue()) do
        ...
        elseif string.byte(k) ~= string.byte("_") then
            debug_write("map_cfg: Copying key:"..k.."->value:"..v..",")
    ...
    if http.formvalue("__apply") then
        ...
        luci.http.redirect(luci.dispatcher.build_url("admin", "network", "multi_ap"))
    end
end
...
function apply_easymesh_bss_cfg(isLocal)
    ...
    if isLocal then
        ...
        luci.http.redirect(luci.dispatcher.build_url("admin", "network", "multi_ap", "easymesh_bss_config_renew"))
    end
end
