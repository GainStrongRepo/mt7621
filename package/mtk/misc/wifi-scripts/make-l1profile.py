...
def validate():
	...
	# when dbdc is enabled, we have more to check
	def tokennum(v):
		return len(v.strip().strip(";").split(";"))
	for i, dx in enumerate([d1,d2,d3]):
		dbdc = tokennum(dx.get("profile_path",""))
		for key in ["init_compatible", "main_ifname", "ext_ifname",
					"wds_ifname", "apcli_ifname", "mesh_ifname", "nvram_zone"]:
			assert dbdc == tokennum(dx.get(key, "")), "dev {0} key {1} dbdc mismatch, expected {2} tokens, got {3}".format(i, key, dbdc, tokennum(dx.get(key, "")))
